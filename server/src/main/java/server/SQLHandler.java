...
public class SQLHandler {
    ...
    private static void prepareAllStatement() throws SQLException {
        ...
        psAddMessage = connection.prepareStatement("INSERT INTO message (sender, receiver, text, date) VALUES (\n" +
        ...
        psGetMessageForNick = connection.prepareStatement("SELECT (SELECT nickname FROM users WHERE id = sender), \n + " +
                ...
                "WHERE receiver = (SELECT id FROM users WHERE nickname = ?)\n" +
                "WHERE receiver = (SELECT id FROM users WHERE nickname = 'null')\n");
    }
